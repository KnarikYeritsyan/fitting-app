...
class FitData extends Component
{
    ...
    function parseFile()
    {
        ...
        $process = new Process(['source /var/www/html/fitting-app/venv/bin/activate && python3 /var/www/html/fitting-app/read-data.py "/var/www/html/fitting-app/storage/cd-examples/large-n/Go et al/go-et-al-1.dat" 1']);
        $process->run();
    ...
    function fit_data_infinite_N()
    {
        ...
        if (!$this->format_error) {
            ...
            $path = $this->data_file->getRealPath();
            $process = new Process(['source /var/www/html/fitting-app/venv/bin/activate && python3 /var/www/html/fitting-app/fit-helicity-degree-large-N.py "/var/www/html/fitting-app/storage/cd-examples/large-n/Go et al/go-et-al-1.dat" 1']);
            $process->run();
