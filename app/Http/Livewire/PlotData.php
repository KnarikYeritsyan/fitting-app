...
//    }
    ...
    function parseFile()
    {
        ...
        $process = new Process([env('PYTHON3_COMMAND','source /var/www/html/fitting-app/venv/bin/activate && python3'),$pythonpath,$path,$this->temperature]);
        $process->run();
    ...
    function fit_data_finite_N()
    {
        ...
        if (!$this->format_error) {
            ...
            $path = $this->data_file_n->getRealPath();
            $process = new Process([env('PYTHON3_COMMAND','source /var/www/html/fitting-app/venv/bin/activate && python3'), $pythonpath, $path, $this->temperature, $this->repeat_units]);
            $process->run();
